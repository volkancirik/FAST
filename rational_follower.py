...
def run_rational_follower(envir, evaluator, follower, speaker, beam_size,
  ...
  while True:
    ...
    if args.debug_rational and batch_idx == 3:
      break
    if include_gold:
    ...
    for instance_index, instance_candidates in enumerate(beam_candidates):
      for i, candidate in enumerate(instance_candidates):
        ...
        candidate['follower_score'] = candidate['score'].cpu()
        # candidate['follower_score'] = np.random.uniform()*-1000  # >>>>
        candidate['speaker_score'] = speaker_scored_candidate['score']
        # candidate['speaker_score'] = np.random.uniform()*-1000  # >>>>
  ...
  print('\nspeaker scores std', speaker_std)
  print('\nfollowe scores std', follower_std)
  accuracies_by_weight = {}
  index_counts_by_weight = {}

  for speaker_weight in [0.0, 0.5, 0.75, 0.95, 0.975, 1.00]:  # Use 0.95 weight
    results = {}
    eval_results = []
    index_count = Counter()
    ...
    score_summary, _, _ = evaluator.score_results(results)
  ...
  if compute_oracle:
    ...
    oracle_score_summary, _, _ = evaluator.score_results(oracle_results)
...
def validate_entry_point(args):
  ...
  for env_name, (val_env, evaluator) in sorted(follower_val_envs.items()):
    ...
    weight, score_summary = max(accuracies_by_weight.items(),
                                key=lambda pair: pair[1][args.metrics])
    print("max {} with weight: {}".format(args.metrics, weight))
