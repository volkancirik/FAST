...
class HallucinationDecoderLSTM0(nn.Module):
  def __init__(self, embedding_size, hidden_size, dropout_ratio,
               feature_size=2048+128+24, image_attention_layers=None,
               visual_hidden_size=1024, num_head=8,
               max_len=80, visual_context_size=-1,
...
class HallucinationDecoderLSTM1(nn.Module):
  def __init__(self, embedding_size, hidden_size, dropout_ratio,
               feature_size=2048+128+24, image_attention_layers=None,
               visual_hidden_size=1024, num_head=8,
               max_len=80, visual_context_size=-1,
    ...
    self.obj_embedding = nn.parameter.Parameter(
        torch.rand(self.n_objects, self.obj_dim)).cuda()
    self.bilinear_size = 32
  ...
  def forward(self, u_t_prev, all_u_t, visual_context, h_0, c_0, ctx,
    ...
    # obj_feats = torch.zeros(batch_size, a_size, self.n_objects).cuda()
    # for aa in range(all_u_t.shape[1]):
    #   obj_feats[:, aa, :] = self.obj_mlp(self.bilinear(visual_context, all_u_t[:, aa, :].unsqueeze(
    #       1).repeat(1, visual_context.shape[1], 1)).sum(1))
...
class HallucinationDecoderLSTM2(nn.Module):
  def __init__(self, embedding_size, hidden_size, dropout_ratio,
               feature_size=2048+128+24, image_attention_layers=None,
               visual_hidden_size=1024, num_head=8,
               max_len=80, visual_context_size=-1,
...
class DeviationMonitor(nn.Module):
  ...
  def forward(self, last_dev, prev_h_t, h_t, c_t, v_ground, t_ground, alpha_v, alpha_t, last_logit):
    input = torch.cat((last_dev.unsqueeze(1), prev_h_t, h_t,
                       v_ground, last_logit.unsqueeze(1)), 1)
...
class DotScorer(nn.Module):
  def __init__(self, traj_embedding_size, instr_embedding_size):
    ...
    self.matching = EltwiseProdScoring(
        traj_embedding_size, instr_embedding_size)
